...
The main structure of the Rif is represented by the limestone massifs. The underground water reserves of the Rif maintain the distribution of vegetation and the rivers that originate in the Rif \cite{Aqil03042015}. Thanks to these hydrogeological features, short coastal rivers and streams flow from the Rif to the Mediterranean, which are characterized by a torrential and erosional regime \cite{Zouhri01062004}. These sources of these rivers supply water to diverse and very heterogeneous types of vegetation around the Rif. These processes briefly illustrate the close link between the geomorphological features, hydrogeology and vegetation of the Rif region. 
...
Mapping of topographic data was carried out using GMT using existing methods \cite{Lemenkova202221,Lemenkova202217,10537274,5369602}, while mapping of land use data was carried out using QGIS. Mapping of land cover maps from remote sensing data is based on satellite image classification methods.
...
This section presents the result of satellite image classification using three approaches: unsupervised classification and supervised classification by machine learning methods using two approaches - "Decision tree classifier" and "Extra tree classifier". Figure \ref{fig05} shows the images generated using the clustering classification method, which illustrate the land cover changes with a bi-monthly time interval, Table \ref{tab01}.
...
Compared to the previous results in Figure \ref{fig05} and Figure \ref{fig06}, the results of the Extra tree are not quite similar for the classification maps based on the “Decision tree” (Figure \ref{fig06}) and “Extra tree” algorithms (Figure \ref{fig07}), because they rely on the extremely random trees method. The “Extra tree” algorithm is similar to the random forest algorithm in its nature, but it processes images much faster. Therefore, technically, this algorithm is an improved ensemble supervised machine learning method.
