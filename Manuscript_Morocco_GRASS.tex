...
The study of the relationship between geomorphology and vegetation has become a major issue for monitoring territories using earth observation data (Boutahar et al., 2023; Brunet 1977 ). In particular, geospatial data is used in the areas of agricultural land management and climate change (Lemenkova and Debeir, 2023a). The production of land use maps on areas with diversified geomorphology is based on satellite data that allow for regular surface photography (Bariou et al., 1984; Ducros-Gambart et al., 1984). These geospatial data are used in classification processes to produce land use maps. 

In Morocco, the use of satellite images for landscape analysis is explained by the very diverse landscape structure (Debarbieux and Me Neill, 1993; El Gharbaoui 1986). Since the geography of Morocco extends from the Atlantic Ocean to mountainous areas and the Sahara desert, the country has a complex geomorphological framework and varied topography. In addition, the geological and tectonic development of the Atlas Mountains and their surroundings presents a complex pattern of geological units (Zarki et al., 2000). This also determines vegetation distribution and controls land use patterns in the mountainous regions of Morocco (Toufiq and Feinberg 1987). This specific context affects the distribution of vegetation over the country and controls land cover types.

In this context, land cover maps characterizing the environmental land cover play a vital role in mapping areas in Morocco. More specifically, since remote sensing information can be interpreted in terms of landscape structural units, comparing multiple satellite images is useful for detecting changes and monitoring landscape dynamics (Amat and Hotyat 1985). However, the question arises regarding the choice of classification algorithms and software for data processing to be provided for classification. The integration of remote sensing data with GIS for environmental mapping has been the subject of research for decades and continues to be a challenging topic. Many approaches to the problem of satellite image analysis have been proposed since the development of satellite missions (Pouquet, 1976; Bardinet, 1981; Regrain, 1988; Joly, 1990; Riopel et al., 2006; Pierrot-Deseilligny and Clery, 2011).

Furthermore, the performance of image classification methods is linked to the quality of remote sensing data. In the field of cartography, Landsat 8-9 OLI/TIRS image data open new opportunities since the spatial resolution of these data is an advantage for determining land use classes. In this context, the launch of new satellite constellations of Landsat 8-9 OLI/TIRS as Earth observation systems allows the acquisition of time series. The acquisition of these data opens new perspectives in cartography.

%----------- section ----------->
\section{Objective and motivation}

This paper focuses specifically on the application of machine learning methods to satellite image processing incorporated into the GRASS GIS software through the embedded Python Scikit-Learning libraries. Machine learning is an artificial approach to geospatial data processing that relies on mathematical and statistical algorithms to give computers the ability to “learn” from data. Machine learning can be applied to satellite images such as Landsat 8-9 OLI/TIRS since pixels are discrete quantitative variables (Lemenkova 2023a; 2023b).

Due to machine learning, the functions of the programs improve their performance in solving geospatial analysis tasks (Johannet et al., 2012). In the field of satellite image processing, this concerns the usual image processing tasks such as pixel recognition, spectral reflectance analysis, classification optimization, the development of more advanced methods of image analysis and interpretation, as well as the implementation of these methods to cartographic visualization, as discussed in previous research (Verger, 1984; Roland, 1980; Pirazzoli, 1982). Compared to traditional approaches, the improvements brought by machine learning algorithms concern the modeling of data with an average statistical error as low as possible.

The GIS GRASS software was chosen as the satellite image processing tool for its combination of efficiency and cross-platform compatibility. This paper uses the GIS GRASS machine learning modules ('r.learn.train', 'r.random', 'r.learn.predict' and 'r.category') with the 'ExtraTreesClassifier' and 'DecisionTreeClassifier' models for supervised applications, training and classification of six satellite images. The results of the machine learning approach to image processing are compared with the results of unsupervised classification performed using the GIS GRASS modules 'i.cluster' and 'i.maxlik'.

In programming libraries, image processing algorithms are implemented as a set of functions used to process raster files, perform geospatial analysis, and create maps in the form of visualized images (Lemenkova, 2024a, 2024b, 2024c; Lemenkova and Debeir, 2022a; 2022b). This is the fundamental approach that differentiates the method of using script libraries from traditional GIS. The use of programming approaches to processing and analyzing satellite images allows handling complex representations of landscape features and detected objects recognized as land cover types that have various properties (extent and heterogeneity of landscapes, its texture, size of plots, fragmentation and topology, etc.).

In particular, in this research, an exploration of GRASS GIS machine learning modules is presented and exploited with the aim of demonstrating the applicability of the spatiotemporal information management approach to environmental research. The scripts of this platform are similar to libraries in programming languages.

%----------- section ----------->
\section{Study Area}

The study focuses on northern Morocco, the Rif Mountains (Fig. 1). The Rif Mountains were formed as a result of complex tectonic interactions between the African and Eurasian plates. Therefore, the complex geological setting of northern Morocco in the Rif region has resulted in moderate tectonic activities (Rampnoux et al., 1979; Benzaggagh, 2000). 
...
The geomorphological structures of the region are composed of three main structural units from north to south: the internal zone, the flysch nappes and the external zone (Martín-Martín et al., 2023a; Chaloüan and Michard, 1985). The crustal thickness beneath the Rif Mountains differs from 36 to 30 km towards the border with the western Mediterranean (Arab et al., 2020).

The Rif is a mountain range that extends across northern Morocco, skirting the Mediterranean Sea and in particular the Alboran Sea on its southern shore. The Rif constitutes a segment of the Mediterranean Alpine ranges. More specifically, it belongs to the Betico-Rifo-Tellian arc in the southwestern Mediterranean (Carbonel et al., 1981). The Rif-Tell belts located in northern Morocco present a young Alpine orogenic system of Morocco that presents various metamorphic complexes formed by geodynamic and tectonic activity in the past (Boudouhou 1997; Michard et al., 2006). This region is characterized by deep sediments that are related to Tethyan rifting (Martín-Martín et al., 2023b; Mouhssine et al., 1999; Marchand 1998), which in turn strongly affect soil properties and vegetation types (Boutahar et al., 2023).

There are very diverse ecosystem types that occupy the landscapes from the foothills, slopes and valleys of limestone mountains to the Saharan sebkhas. The main types of land cover are presented in Fig. 2.
...
The geomorphological structure of the Rif can be divided into three segments. The Western Rif is covered with large mountains; its climate is mild, with heavy rains and showers in winter. The Central Rif has diverse reliefs and heterogeneous landscapes. The Eastern Rif is the flattest part of the range (Guillaume et al., 2018). The primary type is presented by the vegetation of the cultivated lands in mosaic distributed along the basement of the Rif Mountains. The vegetation types in the Atlas Mountains region are presented by a closed to open evergreen or semi-deciduous broadleaf forest, with a forest cover of 100 to 40\% for closed to open trees or semi-deciduous trees. This geomorphological and climatic heterogeneity corresponds to a bio-ecological and environmental diversity. Thus, the spatial richness of the conditions of climate and geology accentuates this biodiversity and types of land use in Morocco.

The main structure of the Rif is represented by the limestone massifs. The underground water reserves of the Rif maintain the distribution of vegetation and the rivers that originate in the Rif. Thanks to these hydrogeological features, short coastal rivers and streams flow from the Rif to the Mediterranean, which are characterized by a torrential and erosional regime. These sources of these rivers supply water to diverse and very heterogeneous types of vegetation around the Rif. These processes briefly illustrate the close link between the geomorphological features, hydrogeology and vegetation of the Rif region. The Rif Mountains region is located between latitudes 34\degree 30'-35°15'N and longitudes 4°30'-5°30'W, with an extension of about 30,000 m and an occupation of 4000 km². This particular geographical position and vast geographical extent between Africa and Europe results in a great variety of ecosystems in Morocco. Landscapes vary from humid in the Rif and Atlas, to arid Saharan in the south of the country, including sub-humid transition zones and semi-arid in the plains and foothills areas (Kettani and Langton, 2012).

Seismic, electrical, magnetic and thermal activities in the Rif caused by local crustal earthquakes are related to the tectonic and geological instability of this region (Serrano et al., 2003). Specifically, this region presents a southern branch of the Betic-Rif arc that is part of the Alpine ranges. This region has experienced deformations between northwest Africa and southern Spain (Iberia) since the Tertiary, followed by the convergence between Africa and the Iberian Peninsula (Reuber et al., 1989). As a result, this tectonic block is expelled towards the west-southwest and the region still experiences seismic and tectonic activities (Fedan and Thomas, 1985). At the same time, seismic-tectonic activities shape the regional geomorphology that, in turn, controls the distribution of land cover types and is expressed in the landscapes. These links between geology, geomorphology and environmental context have been discussed in previous works (Saint Martin and Charriere, 1989; Lemenkova and Debeir, 2022c).

The Mediterranean coastal edge of northern Morocco is a habitat for numerous peatlands with boreal floristic affinities and high conservation value, and wetlands (Muller et al., 2022). Occasional patches of rain-fed trees and scrub crops, which depend on water availability are distributed in the western regions of the country. Rain-fed croplands interspersed with areas of sparse vegetation are visible along the Atlantic Ocean coasts. Sparse herbaceous vegetation comprising a complex pattern mixed with shrubs and thickets. Other land cover types are generally presented by urban areas around cities with artificial surfaces and associated areas, and bare soil areas in desert and unoccupied lands, Figure 2.

In addition to the natural variability of the Rif landscapes, its forest areas are affected by human factors and hazards. Thus, the expansion of agricultural land, forest fires and socio-demographic, economic and urban dynamics negatively affect the mountainous forests of the Rif (Boubekraoui et al., 2023). In addition, the combination of a hot and humid climate, a diverse relief and contrasting topography of the Rif creates favorable conditions for landslides that present geomorphological risks (Rouai and Jaaidi, 2003; Es-smairi et al., 2023).
...
The spatial analysis is based on two types of data: on the one hand, topographic and land use data obtained from GEBCO, (fig. 1) and FAO, (fig. 2); on the other hand, Landsat 8-9 OLI/TIRS satellite image data covering the bimonthly period 2023: January, March, May, July, September and November, figure 3.

The L2 data type is OLI\_TIRS\_L2SP with ground control points version 5. The processing software version is LPGS\_15.3.1c for Landsat-8 scenes and LPGS\_16.3.1 for Landsat-9 (2023) scene. The satellite roll angle is zero for Landsat 8 scenes and -0.001 for Landsat 9. The images were taken during the day at Nadir, with cloud cover less than 10\%.

In this work, only multispectral bands with an acquisition resolution of 30 meters were used, except for panchromatic bands with a resolution of 15 m. The combination of multispectral bands is mainly used for the creation of false/true color composites for classification, especially for image visualization. The available data were listed using the search template in "g.list rast".
...
Mapping of topographic data was carried out using GMT GIS using existing methods (Lemenkova, 2021a; 2021b), while mapping of land use data was carried out using QGIS. Mapping of land cover maps from remote sensing data is based on satellite image classification methods.

The aim of this approach is to model the data and predict for each pixel a label, i.e. Land Cover Class. Using machine learning methods, these methods are divided into two categories: supervised classification and unsupervised classification. This study involves the use and comparison of these two methods. The image processing process is composed of the following steps, implemented in the form of free and open source modules on GRASS SIG.
...
First, the import and preprocessing of six Landsat images was done using the modules 'r.import' that reads the data and 'i.landsat.toar' that converts Landsat 8-9 OLI/TIRS images and transforms the calibrated digital numbers for the reflectance at the top of the atmosphere. In this way, the quality of the images is improved. On the colored composition in "false colors", the vegetation appears in different shades depending on the species, but also on the environmental conditions: geomorphology of the mountain slope, the curvature and the inclination of the slope that affect the shadows on the images, fig. 4.
...
The spectral combination calculation integrates different spectral bands of Landsat based on the physical properties of the land covers reflected in the images. Therefore, the spectral bands are useful to improve the discrimination between landscape classes, (Fig. 4). The vegetation observed on the Atlantic Ocean side appears in darker reds than the surrounding vegetation around the Rif Mountains. The water that absorbs almost all wavelengths is very dark, up to black, while the empty ground surface appears very light, in tones ranging from beige to ochre, Fig. 4.

Creation of color composites of Landsat multispectral bands using the 'r.composite' module and visualization of the images using the 'd.rast' module. The estimation of the color composition model aims to better recognize the ten land cover classes representing landscape patches in the Rif Mountains and its surroundings in the images. Modeling and simulation of the images using the principle of color compositions includes 'true color' and 'false color' images, Fig. 4. Here, the spectral bands of the images are visualized in several color combinations that are widely used in remote sensing because they are very suitable for studying vegetation and landscape. For example, the combination with band number 5 (infrared, IR) is based on the properties of vegetation that reflect very strongly near-IR radiation.
...
In this way, each land cover class is associated with each cluster. Calculating the pixel rejection probability using accuracy assessment for quality control. With the unsupervised modeling done, the new data obtained is submitted as the training dataset for machine learning to obtain better image classification results. The clustering results are used as observation data available for supervised learning. Thus, the results of this step are used for machine learning as the training dataset.

Subsequently, machine learning processing of images was performed using supervised classification that uses a sequence of modules. The ability of machine learning to model geospatial data relies heavily on the acquisition of training data as the algorithm feeds on the input dataset, so this is an important step. Machine learning for image classification using “r.learn.train” and “r.learn.predict” models.

The algorithm is based on the use of the ‘decision tree’ and ‘extra trees’ classifiers that are integrated into the GRASS GIS module ‘r.learn.train’. Here, the data are labeled, i.e. This is a supervised learning with the technical specificities mentioned in note 1. First, the ‘r.random’ module was used to generate training pixels from a previous classification of the land cover of Morocco. All these algorithms were implemented using the GRASS GIS module ‘r.learn.train’ using the programming code explained above.
...
This section presents the result of satellite image classification using three approaches: unsupervised classification and supervised classification by machine learning methods using two approaches - "decision tree classifier" and "additional tree classifier". Figure 5 shows the images generated using the clustering classification method, which illustrate the land cover changes with a bi-monthly time interval, Table \ref{tab01}.
...
Figure 7 shows the land cover type patterns over the Rif Mountains and surrounding areas, defined using an improved machine learning algorithm.
...
Compared to the previous results in Figures 5 and 6, the results of the additional tree are not quite similar for the classification maps based on the “decision tree” and “additional tree” algorithms, because they rely on the extremely random trees method. The “additional tree” algorithm is similar to the random forest algorithm in its nature, but it processes images much faster. Therefore, technically, this algorithm is an improved ensemble supervised machine learning method.

However, this algorithm creates many decision trees using random sampling of pixels for each tree. The most important and unique feature of the “additional trees” algorithm is that it randomly selects a pixel division value. Therefore, compared with the “decision tree” algorithms, the land cover types are diverse and uncorrelated. The best performance among the three image processing methods is demonstrated by the “decision tree” algorithms which divide the image into land cover classes with precision and detail.
...
The general objective of this paper aims to improve the production of land cover maps in northern Morocco from machine learning by GRASS GIS (the algorithms 'decision trees' and 'additional trees') and time series of Landsat 8-9 OLI/TIRS satellite images. In particular, this paper focuses on the comparison of unsupervised and supervised classification (clustering) of remote sensing data (Collet, 1981). At the same time, in the paradigm of information processing systems, the emphasis is on the development of methods (Cauzinille-Marmèche and Mathieu 1994). Therefore, the specific objective of this paper aims to improve land cover mapping in northern Morocco from time series of Landsat satellite images using machine learning methods by GRASS GIS and cartographic scripts.

Two challenges have been identified in this work. The first concerns the technical choice of the classification algorithm (supervised “DecisionTreeClassifier” and “ExtraTreesClassifier” vs unsupervised clustering by MaxLike) using GRASS GIS, while the second concerns the consideration of the identification of seasonal landscape dynamics in the Rif Mountains of Morocco using monitoring data from January to November 2023 and the relationship between vegetation and mountain geomorphology.

This paper demonstrated the implementation of the GRASS GIS machine learning approach to build an object-based classification workflow of six multispectral satellite images covering the northern region of Morocco, in the Rif Mountains. There are several applications of the presented results and the GRASS GIS satellite image processing methods described in this paper. The land cover in the northern region of Morocco presents the ecological, vegetal and geological cover of the land surface.
