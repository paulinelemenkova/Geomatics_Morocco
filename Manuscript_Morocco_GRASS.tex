...
The study of the relationship between geomorphology and vegetation has become a major issue for monitoring territories using Earth observation data \cite{Corenblit,Muir,Pu,Cary01091990}. In particular, geospatial data is used in the areas of agricultural land management and climate change \cite{Ahmadian03052016,Lemenkova202412,IMTIAZ2024109172}. The production of land use maps on areas with diversified geomorphology is based on satellite data that allow for regular surface photography \cite{JOHANSSON01011978}. These geospatial data are used in classification processes to produce land use maps. 

In Morocco, the use of satellite images for landscape analysis is explained by the very diverse landscape structure \cite{Sahrane13122022,Marco04032021,Aouraghe01032010}. Since the geography of Morocco extends from the Atlantic Ocean to mountainous areas and the Sahara desert, the country has a complex geomorphological framework and varied topography. In addition, the geological and tectonic development of the Atlas Mountains and their surroundings presents a complex pattern of geological units \cite{Benvenuti31122024,Ouallali03072020,Khalloufi01032010}. This also determines vegetation distribution and controls land use patterns in the mountainous regions of Morocco \cite{ALIELOMAIRI2024101321}. This specific context affects the distribution of vegetation over the country and controls land cover types.

In this context, land cover maps characterizing the environmental land cover play a vital role in mapping areas in Morocco. More specifically, since remote sensing information can be interpreted in terms of landscape structural units, comparing multiple satellite images is useful for detecting changes and monitoring landscape dynamics \cite{CAI2023103226,Lemenkova202405,XIONG2022109132}. However, the question arises regarding the choice of classification algorithms and software for data processing to be provided for classification. The integration of remote sensing data with GIS for environmental mapping has been the subject of research for decades and continues to be a challenging topic. Many approaches to the problem of satellite image analysis have been proposed since the development of satellite missions \cite{NOY2023103377,Lemenkova202403,Su02062020,Potter18102014,Lemenkova202402}.
...
There are very diverse ecosystem types that occupy the landscapes from the foothills, slopes and valleys of limestone mountains to the Saharan sebkhas. The main types of land cover are presented in Figure \ref{fig02}.
...
The spatial analysis is based on two types of data: on the one hand, topographic and land use data obtained from GEBCO, (Figure \ref{fig01}) and FAO, (Figure \ref{fig02}); on the other hand, Landsat 8-9 OLI/TIRS satellite image data covering the bimonthly period 2023: January, March, May, July, September and November, Figure \ref{fig03}.
...
Mapping of topographic data was carried out using GMT GIS using existing methods \cite{Lemenkova202221,Lemenkova202217}, while mapping of land use data was carried out using QGIS. Mapping of land cover maps from remote sensing data is based on satellite image classification methods.

The aim of this approach is to model the data and predict for each pixel a label, i.e. Land Cover Class. Using machine learning methods, these methods are divided into two categories: supervised classification and unsupervised classification. This study involves the use and comparison of these two methods. The image processing process is composed of the following steps, implemented in the form of free and open source modules on GRASS SIG. The code for organising the project and importing data is presented below.
...
First, the import and preprocessing of six Landsat images was done using the modules 'r.import' that reads the data and 'i.landsat.toar' that converts Landsat 8-9 OLI/TIRS images and transforms the calibrated digital numbers for the reflectance at the top of the atmosphere. In this way, the quality of the images is improved. On the colored composition in "false colors", the vegetation appears in different shades depending on the species, but also on the environmental conditions: geomorphology of the mountain slope, the curvature and the inclination of the slope that affect the shadows on the images, Figure \ref{fig04}.
...
This section presents the result of satellite image classification using three approaches: unsupervised classification and supervised classification by machine learning methods using two approaches - "decision tree classifier" and "additional tree classifier". Figure \ref{fig05} shows the images generated using the clustering classification method, which illustrate the land cover changes with a bi-monthly time interval, Table \ref{tab01}.
...
Compared to the previous results in Figure \ref{fig05} and Figure \ref{fig06}, the results of the additional tree are not quite similar for the classification maps based on the “decision tree” (Figure \ref{fig06}) and “Extra tree” algorithms (Figure \ref{fig07}), because they rely on the extremely random trees method. The “additional tree” algorithm is similar to the random forest algorithm in its nature, but it processes images much faster. Therefore, technically, this algorithm is an improved ensemble supervised machine learning method.

However, this algorithm creates many decision trees using random sampling of pixels for each tree. The most important and unique feature of the “additional trees” algorithm is that it randomly selects a pixel division value. Therefore, compared with the “decision tree” algorithms, the land cover types are diverse and uncorrelated. The best performance among the three image processing methods is demonstrated by the “decision tree” algorithms which divide the image into land cover classes with precision and detail. The accuracy assessment is presented in Figure \ref{fig08} and computational results available in Table \ref{tab03} and Table \ref{tab04}.
